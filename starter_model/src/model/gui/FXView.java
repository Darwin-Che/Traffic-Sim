...
public class FXView extends Application implements ViewInterface {
	...
	public void start(Stage primaryStage) {
		try {
			...
			vertices.setPickOnBounds(false);
	...
	public AnchorPane myControlPanel(Stage primaryStage) {
		...
		// reportPane
		report = new FlowPane(Orientation.VERTICAL);
	...
	class FXVertex extends Group {
		...
		FXVertex(Loc loc, Color color) {
			...
			b.setOnMouseEntered(event -> {
				Label pos = new Label("Loc: " + loc.toString());
				report.getChildren().add(pos);
			});

			b.setOnMouseExited(event -> {
				report.getChildren().remove(1, report.getChildren().size());
			});
	...
	class FXEdge {
		...
		FXEdge(Edge e) {
			...
			this.e = e;
			this.full.setPickOnBounds(true);
			this.part.setPickOnBounds(true);
			this.part.setMouseTransparent(true);
			...
			full.setOnMouseEntered(event -> {
				Label pos = new Label("Edge: " + e.toString());
				Label status = new Label("Current User Number: " + tr.getAllUsers(e).size());
				report.getChildren().addAll(pos, status);
			});
			
			full.setOnMouseExited(event -> {
				report.getChildren().remove(1, report.getChildren().size());
			});
